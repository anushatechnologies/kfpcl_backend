...
public class BuyerOrderController {
    ...
    public ResponseEntity<OrderResponseDto> createOrder(
            ...
            @RequestHeader(value = "UserId", defaultValue = "buyer_1") String buyerId) {
        OrderResponseDto response = orderService.createOrder(orderCreateDto, buyerId, idempotencyKey);
        return ResponseEntity.status(HttpStatus.CREATED).body(response);
    }

    @GetMapping
    public ResponseEntity<List<OrderResponseDto>> getBuyerOrders(@RequestHeader(value = "UserId", defaultValue = "buyer_1") String buyerId) {
